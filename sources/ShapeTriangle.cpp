...
void ShapeTriangle::Shape(const VecDouble& xi, VecInt& orders, VecDouble& phi, MatrixDouble& dphi) {
    ...
    for (int is = 3; is < 6; is++) {
        if (orders[is] == 2) {
            ...
            if (is == 3) {
                phi[count] = 4. * (1.0 - qsi - eta) * qsi;
                dphi(0, count) = 4. * eta;
                dphi(1, count) = 4. * qsi;
            }
            else if (is == 4) {
                phi[count] = 4. * qsi * eta;
                dphi(0, count) = -4. * (-1. + eta + 2. * qsi);
                dphi(1, count) = -4. * qsi;
            }
            else if (is == 5) {
                phi[count] = 4. * (1.0 - qsi - eta) * eta;
                dphi(0, count) = -4. * eta;
                dphi(1, count) = -4. * (-1. + 2. * eta + qsi);
            }
            count++;
        }
    ...
    if (orders[6] == 3) {
        phi[count] = 27. * (1.0 - qsi - eta) * qsi * eta;
        dphi(0, count) = -27. * eta * (-1. + eta + 2. * qsi);
        dphi(1, count) = -27. * qsi * (-1. + 2. * eta + qsi);
        count++;
    }
